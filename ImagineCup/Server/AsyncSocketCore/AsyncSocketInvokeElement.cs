...
namespace AsyncSocketServer
{
    ...
    public class AsyncSocketInvokeElement
    {
        ...
        protected AsyncSocketServer m_asyncSocketServer;//服务器
        protected AsyncSocketUserToken m_asyncSocketUserToken;//客户端（基本单元）
        ...
        protected DateTime m_connectDT;//客户端连接时间
        public DateTime ConnectDT { get { return m_connectDT; } }
        protected DateTime m_activeDT;//客户端活跃时间
        ...
        public AsyncSocketInvokeElement(AsyncSocketServer asyncSocketServer, AsyncSocketUserToken asyncSocketUserToken)
        {
            ...
            m_incomingDataParser = new IncomingDataParser();//协议解析器，用来解析客户端接收到的命令
            m_outgoingDataAssembler = new OutgoingDataAssembler();//协议组装器，用来组织服务端返回的命令

            m_sendAsync = false;

            m_connectDT = DateTime.UtcNow;//connect时间赋值
            m_activeDT = DateTime.UtcNow;//active时间赋值
        }
        /// <summary>
        /// 待子类拓展
        /// </summary>
        ...
        public virtual bool ProcessReceive(byte[] buffer, int offset, int count) 
        {
            ...
            DynamicBufferManager receiveBuffer = m_asyncSocketUserToken.ReceiveBuffer;//缓冲区管理对象

            receiveBuffer.WriteBuffer(buffer, offset, count);//将获取到的数据写入缓冲区（有效的数据包）
            bool result = true;
            while (receiveBuffer.DataCount > sizeof(int))//数据包的前4字节：数据包的长度
            {
                //按照长度分包
                int packetLength = BitConverter.ToInt32(receiveBuffer.Buffer, 0); //获取包长度（前四个字节为数据包的长度）
                if (NetByteOrder)
        ...
        public virtual bool ProcessPacket(byte[] buffer, int offset, int count) 
        {
            ...
            string tmpStr = Encoding.UTF8.GetString(buffer, offset + sizeof(int), commandLen);//取出命令
            if (!m_incomingDataParser.DecodeProtocolText(tmpStr)) //解析命令
              return false;

            return ProcessCommand(buffer, offset + sizeof(int) + commandLen, count - sizeof(int) - commandLen); //处理命令(未看)
        }
        ...
        /// <summary>
        /// 发送数据结束后，回调本函数
        /// 在AsyncSocketServer.ProcessSend进行了回调
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// 心跳包使用了本方法
        /// 检测当前是否正在发送包，如果正在发送，则等待回调
        /// 如果没有正在发送的数据包，则投递发送请求
        /// </summary>
        /// <returns></returns>
