...
namespace AsyncSocketServer
{
    public class ControlSocketProtocol : BaseSocketProtocol
    {
        ...
        {
            ControlSocketCommand command = StrToCommand(m_incomingDataParser.Command);//解析客户端发来的命令
            Console.WriteLine(command);
            Console.Read();
            m_outgoingDataAssembler.Clear();//清空protocolText列表
            m_outgoingDataAssembler.AddResponse();//添加Response字段
            m_outgoingDataAssembler.AddCommand(m_incomingDataParser.Command);//客户端发来的命令
